...
`/guide <number>` Mostra el camí més ràpid des d'on es troba l'usuari fins al restaurant demanat (a través del número amb el que s'identifica a la llista anterior) i mostra instruccions per facilitar el camí a l'usuari
...
**Nota**: Per poder fer la cerca múltiple l'string d'entrada enlloc d'estar separat per espais ha d'estar separat per ";". Exemple: /find pizza;sants.
