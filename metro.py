...
def read_accesses() -> Accesses:
    ...
    for row in df.itertuples():
        ...
        Current_Access = Access( row.ID_ACCES, row.NOM_ACCES ,row.NOM_ESTACIO,
...
def add_lines(m : StaticMap, g: MetroGraph) -> StaticMap:
    ...
    for edge in g.edges():
        ...
        if edge_attributes["type"]== "Tram": # Si l'aresta en la que estem és de tipus Tram, cal pintar-la del color de la línia a la que pertany.
...
def add_nodes(m : StaticMap, g : MetroGraph) -> StaticMap:
    ...
    for node in g.nodes():
        type_node = nx.get_node_attributes(g,"type") # Retorna un diccionari de la forma (identificador_del_node : type)
        if type_node[node] == "Acces":
...
def add_nodes_and_edges_stations(station1: Station, station2: Station, metro_graph: MetroGraph)-> None:
    ...
    if station1._nameline == station2._nameline:  #Si son de la mateixa línia
        metro_graph.add_edge(station1._compound_name, station2._compound_name, type = "Tram", nameline = station1._nameline,
        distance = get_distance(station1._location, station2._location), color = station1._linecolor, coordinates = [station1._location, station2._location]) #Atributs: (tipus d'aresta, linia, distancia, color línia)
    ...
    for station in all_stations:
        ...
        for access in all_accesses:
            if station._id == access._station_id : # Assegurar-nos que l'acces i la estació comparteixen identificador d'estació.
                ...
                metro_graph.add_edge(station._compound_name, access._id, type = "Acces",distance = get_distance(station._location, access._location),
                color = station._linecolor, coordinates = [station._location, access._location]) # Atributs: (tipus d'aresta, distancia, color línia)
...
def add_nodes_accesses(all_accesses: Accesses, metro_graph : MetroGraph)-> None:
    ...
    for access in all_accesses:
        metro_graph.add_node(access._id, type = "Acces", accessibility = access._accessible, coordinates = access._location) # Afegim node de tipus Acces.
    ...
    for station1 in all_stations:
        for station2 in all_stations:
            if station1._nameline != station2._nameline and station1._name == station2._name: #Si les estacions no comparteixen linia però comparteixen nom, son arestes d'enllaç
                ...
                metro_graph.add_edge(station1._compound_name, station2._compound_name, type = "Enllaç", nameline = station1._nameline,
                distance = get_distance(station1._location, station2._location), color = station1._linecolor, coordinates = [station1._location, station2._location]) #Atributs: (tipus d'aresta, linia, distancia, color línia)
...
def get_metro_graph(all_accesses: Accesses, all_stations : Stations) -> MetroGraph:
...
def main():
    ...
    #Obtenim el Graf del Metro de Barcelona
    Metro_Graph = get_metro_graph(all_accesses, all_stations)
